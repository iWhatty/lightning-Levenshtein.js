...
// myers-x-variants\myers_x3.js

"use strict";

// Change from baseline:
// - split charCodeAt from peq lookup: const code = a.charCodeAt(z); const eq = peq[code]
...
export const myers_x3 = (a, b, n, m) => {
  ...
  for (let j = 0; j < vsize - 1; j++) {
    ...
    for (let z = start; z < end; z++) peq[b.charCodeAt(z)] |= 1 << z;

    for (let z = 0; z < n; z++) {
      const word = (z / 32) | 0;
      const code = a.charCodeAt(z);
      const eq = peq[code];
    ...
    for (let z = start; z < end; z++) peq[b.charCodeAt(z)] = 0;
  }
